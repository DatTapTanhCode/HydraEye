...
def get_line_ellipse_point(line_coeffs, x, ellipse_params):
    ...
    """
    intersection_points = find_line_ellipse_intersection(line_coeffs, x, ellipse_params)
    ...
    if intersection_points.shape[0] == 2:
        ...
        if _inbetween(x[0], x[1], intersection_points[0][0]) and not _inbetween(
            x[0], x[1], intersection_points[1][0]
        ):
            return intersection_points[0]
        if _inbetween(x[0], x[1], intersection_points[1][0]) and not _inbetween(
            x[0], x[1], intersection_points[0][0]
        ):
        ...
        for i in range(2):
            for j in range(2):
                distances[i, j] = np.linalg.norm(
                    start_end_points[i] - intersection_points[j]
                )
