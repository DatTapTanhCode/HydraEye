        ...
        for annotation in data_point["annotations"][0]["result"]:
            if annotation["value"]["keypointlabels"][0] == "Start Notch":
                key_point_annotation["start"].append(
                    {k: annotation["value"][k] for k in ("x", "y")}
                )
...
def add_gaussian_to_heatmap(heatmap, x, y, sigma=8, visualize=False):
    ...
    heatmap[img_y_min:img_y_max, img_x_min:img_x_max] = np.maximum(
        scale * gaussian[g_y_min:g_y_max, g_x_min:g_x_max],
...
def generate_heatmap(key_point_list, img=None, size=(448, 448)):
    ...
    for key_point in key_point_list:
        heatmap = add_gaussian_to_heatmap(
            heatmap,
...
def main():
    ...
    for i, annotation in enumerate(annotations):
        ...
        if i % 13 == 0:
            ...
            heatmap_show = generate_heatmap(annotation["middle"], torch.tensor(gray))
...
def read_args():
    ...
    parser.add_argument(
        "--annotation", type=str, required=True, help="json file of annotations"
    )
    parser.add_argument(
        "--directory",
        type=str,
    ...
    parser.add_argument(
        "--size", type=int, required=True, help="Heatmap format is size x size"
    )
