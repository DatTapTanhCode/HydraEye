...
async function fetchCameraReadings() {
    try {
        ...
        Object.keys(readings).forEach(cameraId => {
            ...
            // UPDATE: Store old value before updating
            const oldValue = gaugeReadings[cameraId];
            gaugeReadings[cameraId] = newValue;
            
            console.log(`Backend reading for ${cameraId}: ${newValue} (was: ${previousValue})`);
            ...
            updateCameraDisplay(cameraId, newValue);
            
            // CHECK THRESHOLD IMMEDIATELY FOR REAL DETECTOR CAMERAS
            if (camera && oldValue !== undefined) {
                const threshold = camera.threshold || gaugeConfigs[camera.type]?.normalMax || 0;
                const wasAlerting = oldValue > threshold;
                const isAlerting = newValue > threshold;
                
                if (isAlerting && !wasAlerting) {
                    const assignedMembers = teamMembers
                        .filter(member => member.assignedGauges && member.assignedGauges.includes(cameraId))
                        .map(member => member.name);
                    
                    addAlertLog(cameraId, newValue, threshold, assignedMembers);
                    console.log(`🚨 REAL-TIME ALERT: ${cameraId} exceeded threshold: ${newValue} > ${threshold}`);
                }
            }
        });
    ...
    activeCameras.forEach(camera => {
        ...
        chartContainer.innerHTML = `
            <div class="chart-header">
                <div class="chart-title">${getTypeIcon(camera.type)} ${
      camera.id
    } - ${camera.location}</div>
            ...
            <div class="chart-info">
                <span>Current: <span class="current-reading" id="current-${
                  camera.id
                }">0${gaugeConfigs[camera.type].unit}</span></span>
                <span>Threshold: ${camera.threshold}${
      gaugeConfigs[camera.type].unit
    }</span>
                <span>Status: <span id="chart-status-${
                  camera.id
                }" class="status-normal-reading">NORMAL</span></span>
            </div>
        `;
                        ...
                        <div class="gauge-tags">
                            ${assignedGauges
                              .map((gaugeId) => {
                                ...
                                return `<span class="gauge-tag">${
                                  camera ? camera.id : gaugeId
                                }</span>`;
                              })
                              .join("")}
                        </div>
            ...
            return `
                <div class="log-entry">
                    ...
                    <div class="log-message">
                        <strong>${entry.cameraId}</strong> at ${
          entry.location
        }<br>
                        Threshold exceeded: ${entry.currentValue} > ${
          entry.threshold
        }<br>
...
function updateRecentAlertsTable() {
    ...
    const alertsHTML = recentAlerts
      .map((alert) => {
        const config =
          gaugeConfigs[cameras.find((c) => c.id === alert.cameraId)?.type];
        ...
        return `
                <div class="table-row">
                    ...
                    <div class="time-info">${formatDateTime(
                      alert.timestamp
                    )}</div>
                    <div class="duration-info">Active</div>
                    <div class="threshold-info">${alert.threshold}${unit} → ${
          alert.currentValue
        }${unit}</div>
...
function generateAIPredictions() {
  ...
  const predictions = cameras
    .filter(
      (camera) => (camera.address || camera.demo_mode) && chartData[camera.id]
    )
    .map((camera) => {
  ...
  const predictionsHTML = predictions
    .map((pred) => {
      ...
      const trendIcon =
        pred.trend.direction === "up"
          ? "↗️"
          : pred.trend.direction === "down"
          ? "↘️"
          : "➡️";
      const trendClass =
        pred.trend.direction === "up"
      ...
      return `
            <div class="prediction-card ${pred.status.status}">
                <div class="prediction-header">
                    <div class="prediction-title">${pred.camera.id} - ${
        pred.camera.location
      }</div>
                ...
                <div class="prediction-values">
                    <div class="value-item">
                        ...
                        <div class="value-number">${pred.currentValue.toFixed(
                          1
                        )}${config.unit}</div>
                    </div>
                    <div class="value-item">
                        <div class="value-label">Predicted</div>
                        <div class="value-number">${pred.prediction.toFixed(
                          1
                        )}${config.unit}</div>
                    </div>
                </div>
                <div class="prediction-trend">
                    ...
                    <span>Trending ${
                      pred.trend.direction
                    } (${pred.trend.rate.toFixed(1)}% change)</span>
                </div>
            </div>
        `;
    })
...
function calculateSystemStats() {
  const totalReadings = Object.values(chartData).reduce(
...
function generateSystemSummary(stats) {
  ...
  if (stats.systemHealth >= 90) {
    summary +=
      "<p style='color: #16a34a;'>✅ <strong>Excellent:</strong> Your system is performing optimally with minimal alerts. All cameras are operating within normal parameters.</p>";
  } else if (stats.systemHealth >= 70) {
    summary +=
      "<p style='color: #ca8a04;'>⚠️ <strong>Good:</strong> System performance is generally stable, but there have been some warning events that should be monitored.</p>";
  } else {
    summary +=
      "<p style='color: #dc2626;'>🚨 <strong>Attention Required:</strong> The system has experienced multiple alerts. Immediate review and potential maintenance may be needed.</p>";
  }
...
function updateGaugeReadings() {
    cameras.forEach(camera => {
        let oldReading = gaugeReadings[camera.id];
        let newReading = oldReading;
        
        // Handle demo mode cameras
        if (camera.demo_mode) {
            newReading = generateRandomReading(camera);
            gaugeReadings[camera.id] = newReading;
        ...
        // Handle real detector cameras (if they have readings from backend)
            ...
            newReading = gaugeReadings[camera.id];
            updateCameraDisplay(camera.id, newReading);
        }
        
        // CHECK THRESHOLD ALERTS FOR ALL CAMERAS (not just demo mode)
        if (newReading !== undefined && newReading !== null && oldReading !== undefined) {
            const threshold = camera.threshold || gaugeConfigs[camera.type]?.normalMax || 0;
            const wasAlerting = oldReading > threshold;
            const isAlerting = newReading > threshold;
            ...
            if (isAlerting && !wasAlerting) {
                ...
                addAlertLog(camera.id, newReading, threshold, assignedMembers);
                console.log(`🚨 ALERT: ${camera.id} exceeded threshold: ${newReading} > ${threshold}`);
            }
        }
    });
